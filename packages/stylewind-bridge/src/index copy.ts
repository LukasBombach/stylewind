...
getTailWindClassNames().then(classNames => {
  ...
  for (const className of classNames) {
    ...
    const [, name] = /^(?:.*:)?-?([^-]*)/.exec(className) || ["ERROR", "ERROR"];
    const [, value] = /^-?[^-]+-(.*)/.exec(className) || [];

    values[name] = values[name] || [];

    const propValue = prefixes ? `${prefixes}:${sign}${value}` : `${sign}${value}`;
    // console.log(className.padEnd(25), { [name]: propValue });

    values[name].push(propValue);
    values[name].sort();

    // console.log(className.padEnd(25), sign, (prefixes || "").padEnd(10), name.padEnd(10), value);
  }
